...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp-Camera-Calibration') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-camera-calibration.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-camera-calibration/issues') {
...
function checkReadmes() {
  ...
  if (!readmeJa.startsWith('# TurboWarp-Camera-Calibration\n')) {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/camera-calibration.js')) {
