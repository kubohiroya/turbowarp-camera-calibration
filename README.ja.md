...
| オートフォーカスを切り、カメラ自身の設定でピントを固定する | 連続オートフォーカスは板の動きに合わせて焦点距離を変えます。Camera Sourceはカメラの設定を読むだけで変更しないため、プロジェクトからは固定できません。ピントが動くと、その視点で`capture-condition-mismatch`になりセッションが終わります |
...
| `camera [CAMERA_ID] calibration holdout sample count` | Reporter | 検証用に残したサンプルのうち、実際に検証できた数。0なら何も検証していません |
...
| 自動撮影の1回の撮影が失敗し、セッションは続いている | エラーを記録して撮影を続けます。3回続けて失敗すると停止し、guidanceを空にします |
| ソルバーのworkerが応答しなくなった、またはWebAssemblyモジュールがabortした | 120秒で、またはworkerがエラーを報告した・モジュールがabortした時点で呼び出しを失敗させ、次の呼び出しで新しいworkerを起動します |
| 検証用に残した視点のうち、解いた結果で姿勢を求められたものが2つ未満 | 手動では確かめられなかったとして`reprojection-too-high`。自動撮影は収集を続けます |
...
| セッション中にリサイズ・ズーム・フォーカス・カメラが変わった | 次の視点で、またsolveを確定する前にも`capture-condition-mismatch`。camera-source がこのカメラに合わないと判定するプロファイルになるため、`solved` ではなく `error` で終わる |
| 再投影誤差が上限を超えた | `reprojection-too-high`。プロファイルは保存せず、サンプルを追加できます。手動のsolveも自動撮影と同じく、検証用に2視点以上残したときはfit側に加えてhold-out誤差を確かめます |
