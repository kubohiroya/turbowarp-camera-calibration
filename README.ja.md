...
| プロファイルと異なるサイズのframe、またはプロファイルに記録された設定からリサイズ・ズーム・フォーカス・カメラが変わった状態で板の姿勢を測る | `calibration-not-applicable`。計測せずにカメラを解放します。Camera Sourceが設定をまったく報告しない場合は、これまでどおり計測します |
